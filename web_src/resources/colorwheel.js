...
(function (root, factory) {
  ...
  var ColorWheel = function ColorWheel(options) {
    ...
    this.$.wheel.append("image").attr({
      ...
      height: diameter,
      "xlink:href":
        "https://salvatore-95-solinas:HD7hpxv7jLzCVMp@github.com/salvatore-95-solinas/theLightMaker/blob/main/web_src/resources/resources/wheel.png",
    });
    ...
    return d3.behavior
      ...
      .on("dragend", function () {
        ...
        if (self.currentMode === ColorWheel.modes.ANALOGOUS) {
          var rootTheta = ColorWheel.scientificToArtisticSmooth(
            d3.select(visibleMarkers[0][0]).datum().color.h
          );
          if (visibleMarkers[0].length > 1) {
            var neighborTheta = ColorWheel.scientificToArtisticSmooth(
  ...
  (ColorWheel.prototype.getMarkers = function () {
    ...
    (ColorWheel.prototype.setHarmony = function () {
      ...
      if (!root.empty()) {
        var rootHue = ColorWheel.scientificToArtisticSmooth(
          root.datum().color.h
        );
        switch (this.currentMode) {
          case ColorWheel.modes.ANALOGOUS:
            ...
            this.getVisibleMarkers().each(function (d, i) {
              var newHue =
                (rootHue + ColorWheel.markerDistance(i) * self.slice + 720) %
                360;
  ...
  ColorWheel.prototype.updateHarmony = function (target, theta) {
    ...
    switch (this.currentMode) {
      case ColorWheel.modes.ANALOGOUS:
        this.getVisibleMarkers().each(function (d, i) {
          var startingHue = parseFloat(
            d3.select(this).attr("data-startingHue")
          );
          var slices = 1;
          if (targetDistance !== 0) {
  ...
  ColorWheel.prototype._getColorsAs = function (toFunk) {
    return this.getVisibleMarkers()
      ...
      .map(function (d) {
        return tinycolor({ h: d.color.h, s: d.color.s, v: d.color.v })[
          toFunk
        ]();
      });
  };
  ...
  ColorWheel.artisticToScientificSmooth = function (hue) {
    return hue < 60
      ? hue * (35 / 60)
      : hue < 122
      ? this.mapRange(hue, 60, 122, 35, 60)
      : hue < 165
      ? this.mapRange(hue, 122, 165, 60, 120)
      : hue < 218
      ? this.mapRange(hue, 165, 218, 120, 180)
      : hue < 275
      ? this.mapRange(hue, 218, 275, 180, 240)
      : hue < 330
      ? this.mapRange(hue, 275, 330, 240, 300)
  ...
  ColorWheel.scientificToArtisticSmooth = function (hue) {
    return hue < 35
      ? hue * (60 / 35)
      : hue < 60
      ? this.mapRange(hue, 35, 60, 60, 122)
      : hue < 120
      ? this.mapRange(hue, 60, 120, 122, 165)
      : hue < 180
      ? this.mapRange(hue, 120, 180, 165, 218)
      : hue < 240
      ? this.mapRange(hue, 180, 240, 218, 275)
      : hue < 300
      ? this.mapRange(hue, 240, 300, 275, 330)
